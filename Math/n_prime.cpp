...
pll limits(ll n) {
  // reference in
  // https://docserv.uni-duesseldorf.de/servlets/DerivateServlet/Derivate-28284/pdfa-1b.pdf
  double nn = (double)(n);
  double ln = log(nn);
  double lln = log(ln);

  ll hig = (ll)(nn * (ln + lln - 1 + (lln - 2) / ln -
                      (lln * lln - 6 * lln + 10.273) / (2 * ln * ln)));
  ll low = (ll)(nn * (ln + lln - 1 + (lln - 2) / ln -
                      (lln * lln - 6 * lln + 11.847) / (2 * ln * ln)));
...
void prime_sieve() {
  ...
  for (int i = 2; i < limit; i++) {
    if (!sieve[i]) {
      ...
      pi_sieve[i] = pi_sieve[i - 1];
  }
}

map<pll, ll> phi_cache;

ll phi(ll x, ll a) {
  pll values = {x, a};
  if (phi_cache.find(values) != phi_cache.end())
    return phi_cache[values];
  if (a == 1)
    return (x + 1) / 2;
...
int main() {
  ...
  pll LIMITS = limits(n);
  N = LIMITS.second;
  // optional: use fixed 1e7 and use bitset in prime_sieve
  limit = min((int)(20 * sqrt(2 * N)), 10000000);
  ll lo = LIMITS.first;
  ll hi = LIMITS.second;
  ...
  while (lo < hi) {
    ll me = lo + (hi - lo) / 2;
    ll test = pi(me);
    // optional: comment next if statement, it's a poorly optimization
